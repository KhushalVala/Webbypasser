...
async function bypassYorurl(shortUrl: string) {
  ...
  const client = axios.create({ headers: { 'User-Agent': 'Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36' }, maxRedirects: 0, validateStatus: (s) => s < 400 });
  const { data: html } = await client.get(landingUrl);
  const $ = cheerio.load(html);
  const form = $('#go-link');
  if (!form.length) throw new Error('Form not found');
  const payload: Record<string, string> = {};
  form.find('input').each((_, el) => { const name = $(el).attr('name'); const value = $(el).attr('value') || ''; if (name) payload[name] = value; });
  const postRes = await client.post('https://go.yorurl.com/links/go', new URLSearchParams(payload), { headers: { 'Content-Type': 'application/x-www-form-urlencoded', 'X-Requested-With': 'XMLHttpRequest', Referer: landingUrl } });
...
async function bypassYoulinks(shortUrl: string) {
  ...
  const client = axios.create({ headers: { 'User-Agent': 'Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36' } });
  const { data: html } = await client.get(landingUrl);
  const $ = cheerio.load(html);
  const form = $('#go-link');
  ...
  const payload: Record<string, string> = {};
  form.find('input').each((_, el) => { const name = $(el).attr('name'); const value = $(el).attr('value') || ''; if (name) payload[name] = value; });
  const postRes = await client.post('https://youlinks.in/links/go', new URLSearchParams(payload), { headers: { 'Content-Type': 'application/x-www-form-urlencoded', 'X-Requested-With': 'XMLHttpRequest', Referer: landingUrl } });
  const json = postRes.data;
  if (json.url) {
    const final = await client.get(json.url, { maxRedirects: 5 });
    return { success: true, finalUrl: final.request.res.responseUrl || json.url };
